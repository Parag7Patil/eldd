...
static __init int pchar_init(void)
{
   ...
   printk(KERN_INFO "%s : pchar_create()  is created\n",THIS_MODULE->name);
   ...
   if(ret<0)
   {
         printk(KERN_INFO "%s : the cdev_add is failed\n",THIS_MODULE->name);
	...
	printk(KERN_INFO "%s : pchar_init() is compeleted\n",THIS_MODULE->name);
    return 0;
}
...
static __exit void pchar_exit(void)
{
    ...
    printk(KERN_INFO "%s : pchar_exit() is called\n",THIS_MODULE->name);
	class_destroy(pclass);
	printk(KERN_INFO "%s : class destroy() completed\n",THIS_MODULE->name);
	unregister_chrdev_region(devno,1);
...
static ssize_t pchar_write(struct file *pfile, const char __user *ubuf, size_t size, loff_t *poffset)
{
	  ...
	  avail = MAX - *poffset;
	  printk(KERN_INFO "%s : pcahr_write() : avail bytes = %d\n",THIS_MODULE->name ,avail);
...
MODULE_DESCRIPTION("This is qseudo character device driver");
